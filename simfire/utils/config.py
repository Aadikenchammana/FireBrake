...
class EnvironmentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.typ == "static":
            self.moisture_arr = np.full((self.pixls, self.pixls), self.moisture)
        elif self.typ == "load":
            self.moisture_arr = np.load(self.location)
...
class Config:
    ...
    def _scale_array(self, arr, final_size):
        #t = time.time()
        initial_size = arr.shape[0]
        factor = final_size / initial_size
        scaled_array = np.zeros((final_size, final_size), dtype=arr.dtype)
        for i in range(final_size):
            for j in range(final_size):
                start_x, end_x = int(i / factor), min(int((i + 1) / factor), initial_size)
                start_y, end_y = int(j / factor), min(int((j + 1) / factor), initial_size)
                region = arr[start_x:end_x, start_y:end_y]
                region_mode, _ = mode(region, axis=None)#np.sum(region)/(start_x-end_x)**2#mode(region, axis=None), changed from mode for time conscerns
                scaled_array[i, j] = region_mode if region_mode.size > 0 else 0
        #print("TIMONG1:",t-time.time())  
        return scaled_array
        ...
        if (
            ...
            if self.yaml_data["operational"]["seed"] is not None:
                ...
                if not valid:
                    ...
                    screen_width, screen_height = self.yaml_data["area"]["screen_size"]
                    scaled_fuel = self._scale_array(landfire_lat_long_box.fuel, screen_width)
                    scaled_topo = self._scale_array(landfire_lat_long_box.topography,screen_width)
                    landfire_lat_long_box.topography = scaled_topo
                    landfire_lat_long_box.fuel = scaled_fuel
    ...
    def _load_terrain(self) -> TerrainConfig:
        ...
        if fuel_name is not None and fuel_kwargs is not None:
            ...
            fuel_fn = None
        print("tpypep")
        print(type(topo_layer),type(fuel_layer))
    ...
    def _load_wind(self) -> WindConfig:
        ...
        elif fn_name == "windninja":
            speed_kwargs = None
            dir_kwargs = None
            screen_size_x, screen_size_y = self.yaml_data["area"]["screen_size"]
            height = self.yaml_data["operational"]["height"]
            width = self.yaml_data["operational"]["width"]
            screen_width, screen_height = self.yaml_data["area"]["screen_size"]
            screen_height += 1
            dlat = ((height / 30) * 0.00027777777803598015)
            dlong = ((width / 30) * 0.00027777777803598015)
            x_scale = dlong/width
            y_scale = dlat/height
            tl_lat = self.yaml_data["operational"]["latitude"]
            tl_lon = self.yaml_data["operational"]["longitude"]
            def fitWNdata(tl_lon, tl_lat, x_scale, y_scale, x, y, data_dir):
                with open("WN//data//"+data_dir+"//vel.txt","r") as f:
                    raw_speed = json.loads(f.read())
                with open("WN//data//"+data_dir+"//ang.txt","r") as f:
                    raw_dir = json.loads(f.read())
                with open("WN//data//"+data_dir+"//scaling.txt","r") as f:
                    scaling = json.loads(f.read())
                corr_x = []
                corr_y = []

                for lon_i in range(x):
                    lon = tl_lon +lon_i*x_scale
                    flag = True
                    x_scaling = scaling["x"]
                    for key in x_scaling.keys():
                        if lon < x_scaling[key][1] and lon >= x_scaling[key][0] and flag:
                            corr_x.append(key)
                            flag = False
                for lat_i in range(y):
                    lat = tl_lat - lat_i*y_scale
                    flag = True
                    y_scaling = scaling["y"]           
                    for key in y_scaling.keys():
                        if lat >= y_scaling[key][1] and lat < y_scaling[key][0] and flag:
                            corr_y.append(key)
                            flag = False
                speed_arr = []
                dir_arr = []
                for sy in corr_y:
                    ...
                    dir_row = []
                    for sx in corr_x:
                        speed_row.append(raw_speed[sx+"_"+sy])
                        dir_row.append(raw_dir[sx+"_"+sy])
                    speed_arr.append(speed_row)
                    dir_arr.append(dir_row)
                return np.array(speed_arr),np.array(dir_arr)
            
            speed_arr, direction_arr = fitWNdata(tl_lon, tl_lat, x_scale, y_scale, screen_width, screen_height, self.yaml_data["wind"]["windninja"]["data_dir"])
        elif fn_name == "none":
            speed_kwargs = None
            dir_kwargs = None
            screen_size_x, screen_size_y = self.yaml_data["area"]["screen_size"]
            speed_arr = np.zeros((screen_size_x, screen_size_y))
            direction_arr = np.zeros((screen_size_x, screen_size_y))
        else:
